...
export class ContactsService {
  ...
  async findAll(authUserId: string) {
    ...
    const { data, error } = await this.supabase.admin
      .from('contacts')
      .select('*, contact:users!contacts_contact_user_id_fkey(id, username, display_name, avatar_url, status, last_seen_at)')
      .eq('user_id', userId)
      .neq('status', 'blocked');
  ...
  async sendRequest(authUserId: string, dto: ContactRequestDto) {
    ...
    if (existing) {
      if (existing.status === 'accepted') throw new ConflictException('Already a contact');
      if (existing.status === 'pending') throw new ConflictException('Request already sent');
    }

    const { data, error } = await this.supabase.admin
      .from('contacts')
      .insert({ user_id: userId, contact_user_id: dto.target_user_id, status: 'pending' })
      .select()
      .single();
  ...
  async acceptRequest(authUserId: string, contactId: string) {
    ...
    await this.supabase.admin
      .from('contacts')
      .upsert(
        { user_id: userId, contact_user_id: contact.user_id, status: 'accepted' },
